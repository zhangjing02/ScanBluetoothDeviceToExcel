...
//			arial10format.setBackground(jxl.format.Colour.LIGHT_BLUE);
			...
			arial12format.setBorder(jxl.format.Border.ALL,jxl.format.BorderLineStyle.THIN, Colour.GRAY_25);//给单元格加边
...
//					jxl.format.BorderLineStyle.THIN);
	...
	public static <T> void initExcel(String fileName, List<T> objList) {
		format();
...
//			}
			...
			for (int j = 0; j < objList.size(); j++) {
				...
				for (int i = 0; i < list.size(); i++) {
					sheet.addCell(new Label(j, i , list.get(i),
							arial12format));
				}
			}
			workbook.write();
